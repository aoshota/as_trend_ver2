...
# 仕様
# ±σ2を1回超えたらエントリー（指値注文）
# 逆方向の±σ1を1回超えたら決済（指値注文）
# 同じ方向の±σ2を超えたら損切り（成行注文）
# 指値注文は板情報から金額を設定する
# ローソク足は10分足を使用
...
# bband = {"mean":,"upper1":,"lower1":,"upper2":,"lower2":}
def CalcBB(data):
	bband = {}
	bband["mean"] = sum(data) / len(data)
	bband["upper1"] = bband["mean"] + np.std(data) * 1
	bband["lower1"] = bband["mean"] + np.std(data) * (-1)
	bband["upper2"] = bband["mean"] + np.std(data) * 2
	bband["lower2"] = bband["mean"] + np.std(data) * (-2)
	return bband
...
def CheckAndEntry(data_now,element):
  # 現在のレートが+σ2より大きい時、SELL
  if int(data_now["last"]) >= element["data_bb"]["upper2"]:
    element["flag_position"] = "SELL"
    element = Entry(data_now,element)
  # 現在のレートが-σ2より小さい時、BUY
  if int(data_now["last"]) <= element["data_bb"]["lower2"]:
...
def CheckAndSettlement(data_now,element):
  # ロングポジションを入れている時、現在のレートが+1σより大きかったら決済、-σ2より小さかったら損切り(成行注文)
  if element["flag_position"] == "BUY":
    if int(data_now["last"]) <= element["data_bb"]["upper1"]:
      element = Settlement(data_now,element)
      element["flag_position"] = "NO"
    if int(data_now["last"]) <= element["data_bb"]["lower2"]:
      element = StopLoss(element)
      element["flag_position"] = "NO"
  # ショートポジションを入れている時、現在のレートが-1σより小さかったら決済、+σ2より大きかったら損切り(成行注文)
  elif element["flag_position"] == "SELL":
    if int(data_now["last"]) >= element["data_bb"]["lower1"]:
      element = Settlement(data_now,element)
      element["flag_position"] = "NO"
    if int(data_now["last"]) >= element["data_bb"]["upper2"]:
      element = StopLoss(element)
...
while True:
  ...
  if element["data_bb"] and element["flag_bb_20"]: # BBのデータがあり最新の指定時間のデータを取得している場合エントリー・決済条件を検査する
